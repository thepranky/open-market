...
1. **Spec** in `docs/specs/YYYY-MM-DD-name.md` — goal, approach, files, verification.
2. **Small PR** — one spec, one change set.
3. **Verify** — every command/check from the spec.

**DDRs** (`docs/architecture/decisions/`) — for understanding subsystems; reference when changing that area.
