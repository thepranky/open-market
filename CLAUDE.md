...
4. **Independent review** — use `/ship` which handles this automatically: it spawns a cold Sonnet sub-agent (uncorrelated judgement — same reason the pipeline favours independent extraction) that invokes `/code-review` at an effort level matched to the diff size, then returns findings for triage. Fix blockers, report what you accepted or rejected and why.
