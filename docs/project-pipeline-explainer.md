...
**Current state:** This is a v0 first slice. There are 18 source-verified canonical case records. The ingestion pipeline now covers source caching, multi-focus LLM extraction, deterministic validation, LLM review triage, safe draft-to-canonical promotion, review-learning logs, eval fixtures, and long-decision assembly. The pipeline started with market-definition extraction and has now been extended to outcome metadata, theories of harm, remedies/commitments, and repeated-unit assessment for long Phase II decisions. The long-decision pipeline has been proven on a promoted Phase II conditional-clearance hard case (`eu_bayer_monsanto_2018`), controlled corpus expansion has produced a clean EC Phase I case (`eu_cochlear_oticon_medical_2023`), the first same-transaction EC/CMA Phase II pair (`eu_viasat_inmarsat_2023` and `uk_viasat_inmarsat_2023`), the profile-driven US court-opinion path has been proven on `us_tapestry_capri_2024`, and the one-command controlled-case runner has promoted `eu_booking_etraveli_2023` from merged draft to canonical data. Frontend cleanup is deferred.
...
- **Status:** Implemented and verified after the data baseline pass. The current 18-case dataset seeds cleanly into Neo4j. Optional — the API falls back to YAML-based queries if Neo4j is unavailable.
...
| **Next** | **Broad case index** ← Current | Create a cheaper `data/case_index/` layer for broad coverage: case metadata, parties, authority, date, outcome, procedure stage, sector/source links, document status, and quality level. Run deterministic URL/source/cache checks before any LLM extraction. | **Claude + human + ChatGPT** — Claude implements index schema/scripts; human validates source policy and priority corpus; ChatGPT helps keep the indexing strategy product-focused | Gives the platform broad discoverability quickly and cheaply without pretending every case is a fully reviewed canonical record |
| **Next+1** | **Concept graph from index/cards** | Extend graph seeding so indexed and canonical cases both connect to meaningful concept nodes: parties, sectors, outcomes, authority, procedure stage, market concepts, theory concepts, remedy concepts, and source documents. Label edges by quality level (`keyword_detected`, `source_backed_summary`, `canonical_reviewed`). | **Claude + human** — Claude implements graph/index integration; human defines concept taxonomy and quality labels | Makes the graph useful before every case is fully extracted, while preserving trust by distinguishing summary-derived links from canonical propositions |
...
>
> Today we have 18 source-verified canonical case records, including the first promoted long Phase II conditional-clearance hard case, the first same-transaction EC/CMA Phase II pair, the first promoted US federal court opinion, and the first case promoted through the one-command controlled-case runner. The pipeline began with market-definition extraction, but it now supports a more complete merger-control record: outcome metadata, theories of harm, remedies and commitments, source passages, and repeated unit-level findings for long decisions. For large Phase II decisions, we can run narrow page-range passes and merge them into one reviewable draft before a human promotes anything to canonical data.
>
