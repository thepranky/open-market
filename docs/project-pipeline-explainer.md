...
**Current state:** This is a v0 first slice. There are 17 source-verified canonical case records. The ingestion pipeline now covers source caching, multi-focus LLM extraction, deterministic validation, LLM review triage, safe draft-to-canonical promotion, review-learning logs, eval fixtures, and long-decision assembly. The pipeline started with market-definition extraction and has now been extended to outcome metadata, theories of harm, remedies/commitments, and repeated-unit assessment for long Phase II decisions. The long-decision pipeline has been proven on a promoted Phase II conditional-clearance hard case (`eu_bayer_monsanto_2018`), controlled corpus expansion has produced a clean EC Phase I case (`eu_cochlear_oticon_medical_2023`) plus the first same-transaction EC/CMA Phase II pair (`eu_viasat_inmarsat_2023` and `uk_viasat_inmarsat_2023`), and the profile-driven US court-opinion path has been proven on `us_tapestry_capri_2024`. Frontend cleanup is deferred.
...
- **Status:** Implemented and verified after the data baseline pass. The current 17-case dataset seeds cleanly into Neo4j. Optional — the API falls back to YAML-based queries if Neo4j is unavailable.
...
| **Scalability**                 | 3/10      | In-memory YAML search works for 17 cases; Neo4j graph seeds cleanly but the corpus is still too small              | Add proper database indexing and move search to Neo4j full-text; increase case count to 50+ |
...
| **User-facing usefulness**      | 5/10      | Frontend exists and shows markets, theories, sources; but 17 cases is still too few for real research value; frontend cleanup still needed | Expand case count; add cross-case filtering by market name and theory type                  |
...
| **8h**   | **One-command controlled-case runner** ✅ Done | `apps/api/scripts/run_controlled_case.py` — thin orchestrator over existing scripts; wraps seed creation, source fetch/cache, profile selection, coverage planning, focused extraction, draft merging, and readiness packet into one command; produces READY_FOR_HUMAN_REVIEW or NOT_READY with actionable blockers and git hygiene warnings; 51 targeted tests, no auto-promote | **Claude + human** — Claude implemented orchestrator/tests; human validates workflow and promotion | Turns the 7-step script chain into a single terminal command; operator runs one command per new case and receives either a promotion-ready packet or a blockers list |
...
>
> Today we have 17 source-verified canonical case records, including the first promoted long Phase II conditional-clearance hard case, the first same-transaction EC/CMA Phase II pair, and the first promoted US federal court opinion. The pipeline began with market-definition extraction, but it now supports a more complete merger-control record: outcome metadata, theories of harm, remedies and commitments, source passages, and repeated unit-level findings for long decisions. For large Phase II decisions, we can run narrow page-range passes and merge them into one reviewable draft before a human promotes anything to canonical data.
>
