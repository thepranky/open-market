...
**Current state:** This is a v0 first slice. There are 16 source-verified canonical case records. The ingestion pipeline now covers source caching, multi-focus LLM extraction, deterministic validation, LLM review triage, safe draft-to-canonical promotion, review-learning logs, eval fixtures, and long-decision assembly. The pipeline started with market-definition extraction and has now been extended to outcome metadata, theories of harm, remedies/commitments, and repeated-unit assessment for long Phase II decisions. The long-decision pipeline has been proven on a promoted Phase II conditional-clearance hard case (`eu_bayer_monsanto_2018`), and controlled corpus expansion has now produced a clean EC Phase I case (`eu_cochlear_oticon_medical_2023`) plus the first same-transaction EC/CMA Phase II pair (`eu_viasat_inmarsat_2023` and `uk_viasat_inmarsat_2023`). Frontend cleanup is deferred.
...
- **Status:** Implemented and verified after the data baseline pass. The current 16-case dataset seeds cleanly into Neo4j. Optional — the API falls back to YAML-based queries if Neo4j is unavailable.
...
| **Scalability**                 | 3/10      | In-memory YAML search works for 16 cases; Neo4j graph seeds cleanly but the corpus is still too small              | Add proper database indexing and move search to Neo4j full-text; increase case count to 50+ |
...
| **User-facing usefulness**      | 5/10      | Frontend exists and shows markets, theories, sources; but 16 cases is still too few for real research value; frontend cleanup still needed | Expand case count; add cross-case filtering by market name and theory type                  |
...
| **Next** | **Review-readiness architecture** ← Current | Add deterministic coverage/readiness checks so one controlled-expansion run produces a near-review-ready packet and flags missing geographic markets, missing theory sections, unset source roles, duplicate passages, and skipped planned sections before human review | **Claude + human + ChatGPT** — Claude implements the scripts/tests; human validates legal policy; ChatGPT helps define failure modes and review packet design | Reduces the current 3–4 prompt repair loop per case and makes controlled corpus expansion cheaper and safer |
...
>
> Today we have 16 source-verified canonical case records, including the first promoted long Phase II conditional-clearance hard case and the first same-transaction EC/CMA Phase II pair. The pipeline began with market-definition extraction, but it now supports a more complete merger-control record: outcome metadata, theories of harm, remedies and commitments, source passages, and repeated unit-level findings for long decisions. For large Phase II decisions, we can run narrow page-range passes and merge them into one reviewable draft before a human promotes anything to canonical data.
>
