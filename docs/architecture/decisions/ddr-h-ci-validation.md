...
> Audit CompMap's test and CI story. What runs on every PR vs nightly vs manual? Map each validation script to when it should run. List tests that exist but aren't in CI. Explain the eval benchmark pass criteria. What would you add to PR CI first and why? How does this relate to promotion gates vs merge gates?
