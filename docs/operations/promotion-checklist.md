...
apps/api/.venv/bin/python apps/api/scripts/cases/extract/ingest_case.py \
    --case-id {case_id} --focus market_definition --no-claude

# 2. Source integrity gate (no errors permitted)
apps/api/.venv/bin/python apps/api/scripts/cases/integrity/check_source_integrity.py \
