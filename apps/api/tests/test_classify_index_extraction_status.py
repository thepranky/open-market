...
def test_missing_pdf_url_is_pending():
    assert classify_entry(_entry(pdf_url=None), canonical_exists=False,
                          page_count_fn=_const_pages(2), max_simplified_pages=3) == "pending"


def test_failed_fetch_is_pending():
    # A None page count (fetch/parse failure) must not be treated as simplified.
    assert classify_entry(_entry(), canonical_exists=False,
                          page_count_fn=lambda url: None, max_simplified_pages=3) == "pending"
