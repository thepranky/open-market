...
Usage (from repo root):
    python apps/api/scripts/cases/run_controlled_case.py \\
...
def write_run_report(
    ...
    if result.status == READY:
        ...
        lines += [
            ...
            f"    python apps/api/scripts/cases/run_controlled_case.py --case-id {result.case_id} [original flags]",
        ]
    else:
        lines += [
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if getattr(args, "promote", False):
        print(
            ...
            f"  python apps/api/scripts/cases/promote_case_pipeline.py --case-id {args.case_id}",
            file=sys.stderr,
        )
