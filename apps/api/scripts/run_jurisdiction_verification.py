...
def main() -> int:
    ...
    failures += _run(
        "Completeness gate",
        [PYTHON, "scripts/verify_jurisdiction_completeness.py", "--json"],
    )
    failures += _run(
    ...
    if args.tier in ("nightly", "full"):
        passage_cmd = [PYTHON, "scripts/verify_jurisdiction_passages.py", "--json"]
        ...
        staleness_cmd = [PYTHON, "scripts/monitor_jurisdiction_staleness.py", "--json"]
        if args.jurisdiction:
