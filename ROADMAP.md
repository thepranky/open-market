...
| | 4.5 | Case YAML semantic lint | `validator.py` or new script | Lawyer rules not enforced by Pydantic | [spec](docs/specs/2026-06-25-case-semantic-lint.md): deterministic gate — complaint-only markets not `defined`; dangling `supports_*` refs. Outcome-passage rule dropped (`section` populated on 9/7566 passages; belongs to Stage 5a critic + dual extraction 5.9) |
