...
| | 3.8 | CI robustness follow-ups (deferred by #21) | `api-ci.yml`, `data-contracts.yml`, `jurisdiction-verification.yml`, `pyproject.toml` | #21 closed 3.5–3.7 but flagged robustness gaps as explicit non-goals (recorded here so they're not lost) | Per [ci-gaps spec](docs/specs/2026-06-25-ci-gaps.md) §"Is CI complete?": **A** path-filter `api-ci.yml` (`apps/api/**`,`data/**`); **B** `concurrency: cancel-in-progress` on the two data workflows (done for web/api-ci); **C** pip caching (`actions/cache`); **F** `ruff format --check`. Each small + independent. **D** frontend tests and **E** benchmark F1 trend assertion are larger, intentional non-goals (E ties 5.3/5.5) — no separate row |
