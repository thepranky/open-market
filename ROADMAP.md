...
| | 5.10 | Multi-jurisdiction PDF resolution | `resolve_eu_pdf_urls.py`, new per-authority resolvers, `ingest_case.py --from-index` | Only EU Phase I auto-resolves; Phase II / UK / US need manual URLs (ddr-b gap) | Shared resolver interface; per-authority adapters (EUR-Lex, CMA, FTC/DOJ); dedup with EU path |
