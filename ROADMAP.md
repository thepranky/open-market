...
| | ✅ 5.10 | Multi-jurisdiction PDF resolution | `pdf_resolvers.py`, `resolve_case_index_pdf_urls.py`, `ingest_case.py --from-index` | Only EU Phase I auto-resolves; Phase II / UK / US need manual URLs (ddr-b gap) | ✅ Done — [spec](docs/specs/completed/2026-06-26-multi-jurisdiction-pdf-resolution.md): one resolver contract + EU Cellar / UK GOV.UK / US DOJ-FTC adapters; shared batch CLI with surgical `pdf_url` write; `ingest_case --from-index` and batch share one path; old per-jurisdiction scripts kept as deprecated wrappers. Adapters prefer a correct `manual_required` over a wrong URL |
